...
*/
	...
	public:
		...
		CVector3f* vpVertices;		// array of vertices 
		unsigned int nNormals;		// number of normals in model 
		CVector3f* vpNormals;		// array of normals 
		unsigned int nTexCoords;	// number of texcoords in model 
		CVector3f* vpTexCoords;		// array of texture coordinates 
		unsigned int nFacetnorms;	// number of facetnorms in model 
		CVector3f* vpFacetNorms;		// array of facetnorms 
		...
		CVector3f position;		// position of the model 
...
protected:
	CVector3f m_vMax, m_vMin;

	void CalcBoundingBox();
	bool Equal(CVector3f * u, CVector3f * v, float epsilon);
...
public:
	void Destory();
	void Boundingbox(CVector3f &vMax, CVector3f &vMin);
