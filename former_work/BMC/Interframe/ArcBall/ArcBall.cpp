...
void CArcBall::SaveBinary(FILE *fp)
{
	fwrite (&mId, sizeof(HMatrix), 1, fp);
	fwrite (otherAxis[0], sizeof(float), 4, fp);
	fwrite (&qOne, sizeof (Quat_t), 1, fp);
	fwrite (&ball_data, sizeof(BallData), 1, fp);
	fwrite (&ball_matrix, sizeof(HMatrix), 1, fp);
}

void CArcBall::LoadBinary(FILE *fp)
{
	fread (&mId, sizeof(HMatrix), 1, fp);
	fread (otherAxis[0], sizeof(float), 4, fp);
	fread (&qOne, sizeof (Quat_t), 1, fp);
	fread (&ball_data, sizeof(BallData), 1, fp);
	fread (&ball_matrix, sizeof(HMatrix), 1, fp);
}
